// At the VERY TOP of script.js (before any event listeners)
const auth = firebase.auth();
const firestore = firebase.firestore();

// Optional: Add auth state listener to verify initialization
  ...
  document.querySelectorAll(".apply-btn").forEach((btn) => {
    btn.addEventListener("click", async function () {
      try {
        ...
        alert(`Application submitted for "${jobDoc.data().title}"`);
      } catch (error) {
...
document.addEventListener("DOMContentLoaded", setupApplicationSystem);
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const createFreelancerProfileBtn = document.getElementById(
    "createFreelancerProfileBtn"
  );
  const browseJobsBtn = document.getElementById("browseJobsBtn");
  const freelancerProfileForm = document.getElementById(
  ...
  function loadFreelancers() {
    freelancersCollection
      ...
      .then((querySnapshot) => {
        ...
        if (
          !clientView.classList.contains("hidden") &&
          !freelancersContainer.classList.contains("hidden")
        ) {
          displayFreelancers();
        }
      })
  ...
  function loadJobs() {
    jobsCollection
      ...
      .then((querySnapshot) => {
        ...
        if (
          !freelancerView.classList.contains("hidden") &&
          !jobListings.classList.contains("hidden")
        ) {
          displayJobs();
        }
      })
  ...
  function setupEventListeners() {
    ...
    // Freelancer actions
    createFreelancerProfileBtn.addEventListener(
      "click",
  ...
  async function saveFreelancerProfile(e) {
    ...
    try {
      ...
      const freelancerData = {
        ...
        skills: document
          .getElementById("freelancerSkills")
          .value.split(",")
        ...
        experience:
          document.getElementById("freelancerExperience").value + " years", // FIXED: was using freelancerRate instead of freelancerExperience
        photo: photoUrl,
  ...
  function initSearch() {
    ...
    document
      ...
      .addEventListener("submit", function (e) {
        ...
        const durationFilter =
          document.getElementById("jobDurationFilter").value;
    ...
    document
      .getElementById("freelancerSearchForm")
      .addEventListener("submit", function (e) {
        e.preventDefault();
        const searchTerm = document
          .getElementById("freelancerSearchInput")
          .value.toLowerCase();
        const rateFilter = document.getElementById(
          "freelancerRateFilter"
        ).value;
        const experienceFilter = document.getElementById(
  ...
  function displayJobs(jobsToDisplay = jobs, searchTerm = "") {
    ...
    jobsToDisplay.forEach((job) => {
      ...
      article.innerHTML = `
                ...
                <p class="description">${highlightSearchTerms(
                  job.description,
                  searchTerm
                )}</p>
                <p class="budget">${highlightSearchTerms(
                  job.budget,
                  searchTerm
                )}</p>
                <p class="duration">${highlightSearchTerms(
                  job.duration,
                  searchTerm
                )}</p>
                <menu class="skills">
                    ${job.skills
                      .map((skill) => {
                        const highlightedSkill = highlightSearchTerms(
                          skill,
                          searchTerm
                        );
      ...
      applyBtn.addEventListener("click", async () => {
        try {
          ...
          // 2. Get user document (FIXED: properly declare userDoc)
          ...
          // 3. Get job document (FIXED: properly declare jobDoc)
          const jobDoc = await firestore.collection("jobs").doc(job.id).get();
          if (!jobDoc.exists) {
          ...
          await firestore.collection("applications").add({
            freelancerId: user.uid,
            // freelancerName: userDoc.data().name || "Anonymous",
            jobId: job.id,
            jobTitle: jobDoc.data().title || "Untitled Position",
            status: "pending",
            appliedAt: firebase.firestore.FieldValue.serverTimestamp(),
          });

          alert(`Successfully applied for "${jobDoc.data().title}"`);
        } catch (error) {
          console.error("Application Error:", {
            error: error.message,
            user: auth.currentUser?.uid,
  ...
  function displayFreelancers(
    freelancersToDisplay = freelancers,
    searchTerm = ""
  ) {
    ...
    freelancersToDisplay.forEach((freelancer) => {
      ...
      article.innerHTML = `
                ${
                  freelancer.photo
                    ...
                    : '<figure class="profile-photo empty"></figure>'
                }
                <h3>${highlightSearchTerms(freelancer.name, searchTerm)}</h3>
                <p class="title">${highlightSearchTerms(
                  freelancer.title,
                  searchTerm
                )}</p>
                <p class="rate">${highlightSearchTerms(
                  freelancer.rate,
                  searchTerm
                )}</p>
                ${
                  freelancer.bio
                    ? `<p class="bio">${highlightSearchTerms(
                ...
                <p class="location">${highlightSearchTerms(
                  freelancer.location,
                  searchTerm
                )}</p>
                <p class="experience">${highlightSearchTerms(
                  freelancer.experience,
                  searchTerm
                )}</p>
                <button class="hire-btn" data-id="${
                  freelancer.id
                }">Contact</button>
            `;
      ...
      hireBtn.addEventListener("click", () => {
        alert(
          `Contact feature for "${freelancer.name}" will be implemented in the next phase.`
        );
